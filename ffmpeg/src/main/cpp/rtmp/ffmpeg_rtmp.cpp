...
#define loge(content)   __android_log_write(ANDROID_LOG_WARN,"ffmpeg",content)
#define logd(content)   __android_log_write(ANDROID_LOG_DEBUG,"ffmpeg",content)
...
JNIEXPORT jint JNICALL Java_com_jiangyt_library_ffmpeg_FFMpegRtmp_setCallback
        (JNIEnv *env, jobject instance, jobject pushCb) {
    // 转换为全局变量
    pushCallback = env->NewGlobalRef(pushCb);
    if (pushCallback == NULL) {
        return -3;
    }
    ...
    env->CallVoidMethod(pushCallback, mid, (jlong) 0, (jlong) 0, (jlong) 0,
                        (jlong) 0);
    return 0;
}
...
JNIEXPORT jint JNICALL Java_com_jiangyt_library_ffmpeg_FFMpegRtmp_pushRtmpFile
    ...
    //输出的地址
    const char *outUrl = "rtmp://10.58.238.36:8935/stream/live_test";
...
JNIEXPORT jint JNICALL Java_com_jiangyt_library_ffmpeg_FFMpegRtmp_onFrameCallback
        (JNIEnv *env, jobject instance, jbyteArray buffer_) {
...
//                        (long long) ((av_gettime() - startTime) / 1000));
    ...
    enc_pkt.pts = f_count * (video_st->time_base.den) / ((video_st->time_base.num) * fps);
    enc_pkt.dts = enc_pkt.pts;
    enc_pkt.duration = (video_st->time_base.den) / ((video_st->time_base.num) * fps);
    FFLOGI("index:%d,pts:%lld,dts:%lld,duration:%lld,time_base:%d,%d",
           f_count,
